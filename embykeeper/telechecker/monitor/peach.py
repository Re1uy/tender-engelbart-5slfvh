...
class PeachMonitor(Monitor):
    ocr = "digit5-peach@v1"
    lock = asyncio.Lock()

    name = "Peach"
    chat_name = "peach_emby_chat"
    chat_keyword = r"Peach-register-[A-Za-z0-9-]+"
    ...
    async def init(self, initial=True):
        ...
        Peach_monitor_pool[self.client.me.id] = self
        self.captcha = None
        self.log.info(f"正在初始化 {self.name} 机器人状态.")
        wr = async_partial(self.client.wait_reply, self.bot_username)
        peach_locks.setdefault(self.client.me.id, asyncio.Lock())
        lock = peach_locks.get(self.client.me.id, None)
